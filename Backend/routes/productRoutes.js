...
// @desc Retrieve highest rated product
// @access Public
// ======================================================
router.get("/best-seller", async (req, res) => {
  try {
    const bestSeller = await Product.findOne()
      .select(
        "name price images rating brand category collections gender countInStock"
      )
      .sort({ rating: -1 })
      .lean();
...
router.get("/new-arrivals", async (req, res) => {
  try {
    const newArrivals = await Product.find()
      .select(
        "name price images rating brand category collections gender countInStock createdAt"
      )
      .sort({ createdAt: -1 })
      .limit(8)
      .lean();
...
router.get("/", async (req, res) => {
  try {
    ...
    const products = await Product.find(query)
      .select(
        "name price images gender category brand collections sizes colors rating countInStock"
      )
      .sort(sort)
